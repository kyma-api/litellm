    ...
    pub async fn connect_url(
        ...
        let result = match timeout {
            Some(timeout) => tokio::time::timeout(timeout, connect).await.map_err(|_| {
                Error::transport("Responses WebSocket connection timed out".to_string())
            })?,
            None => connect.await,
        };
        let (socket, _) = result.map_err(|error| match error {
            tokio_tungstenite::tungstenite::Error::Http(response) => {
                Error::upstream(response.status().as_u16(), String::new())
            }
...
async fn dial_upstream(
    ...
    .await
    .map_err(|_| Error::transport("Responses WebSocket connection timed out".to_string()))?;
    result
        .map(|(socket, _)| socket)
    ...
    loop {
        tokio::select! {
            ...
            message = upstream_rx.next() => {
                ...
                match message.map_err(|error| Error::transport(error.to_string()))? {
                    Message::Text(text) => {
