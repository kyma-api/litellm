...
fn document_content_item(document: &Value) -> Result<Value, Error> {
    ...
    let url = object
        ...
        .and_then(Value::as_str)
        .filter(|value| !value.is_empty())
        .ok_or(Error::missing_field(url_field))?;
...
fn first_choice_content(response: &Value) -> Result<Value, Error> {
    response
        ...
        .ok_or_else(|| Error::invalid_response("No content in DeepSeek OCR response".to_string()))
}
...
impl OcrProviderConfig for VertexAiDeepSeekOcrConfig {
    ...
    fn transform_ocr_response(
        ...
        let response = response_json.as_object().ok_or_else(|| {
        ...
        let object = ocr_data
            .as_object()
            .ok_or_else(|| Error::invalid_response_type("object", json_type_name(&ocr_data)))?;
        let pages = object
            .get("pages")
            .and_then(Value::as_array)
            .cloned()
